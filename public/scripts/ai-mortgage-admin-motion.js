...
(function () {
  ...
  // ========== INCLUDED workflow — reading-bar animates across cards in sequence ==========
  // Pauses when scrolled out of view, resumes from the same point when back in view.
  ...
  // ========== HERO widget parallax (mouse) ==========
  // Uses the `translate` CSS property so it composes with each card's base
  // `transform` (rotations on alert/todo, translateX(-50%) centering on wa).
  if (!prefersReduced && window.matchMedia('(pointer: fine)').matches) {
